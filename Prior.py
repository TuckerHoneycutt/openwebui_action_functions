...
class Action:
    ...
    async def action(
        ...
        if action_type == "upload_template" and TEMPLATE_SUPPORT:
            ...
            return await self._handle_template_generation(
                body, __user__, __event_emitter__, __event_call__
            )
    ...
    async def _handle_template_upload(
        self, body: dict, __user__, __event_call__
    ) -> dict:
        """Handle template upload"""
    ...
    async def _handle_template_generation(
        self, body: dict, __user__, __event_emitter__, __event_call__
    ):
        ...
        if not template_name:
            ...
            final_html_body = (
                f"{department_header}{intro_html}{html_content}{footer_html}"
            )
                        ...
                        await __event_call__(
                            {
                                "type": "execute",
                                "data": {
                                    "code": self._get_download_script(
                                        pdf_data, filename
                                    )
                                },
                            }
        ...
        try:
            ...
            if __event_call__:
                await __event_call__(
                    {
                        "type": "execute",
                        "data": {"code": self._get_download_script(pdf_data, filename)},
                    }
                )
    ...
    def _overlay_content_on_pdf_template(
        ...
        try:
            ...
            for page_num in range(len(template_doc)):
                ...
                output_page.show_pdf_page(template_page.rect, template_doc, page_num)
                ...
                for block in content_blocks:
                    if page_num == 0:  # Only add content to first page for now
                        if block["type"] == "heading":
                            ...
                            # Estimate height
                            y_position += (
                                len(block["text"]) / 80 * block.get("size", 11)
                            )

            # Save output
            output_path = os.path.join(
                self.pdf_generator.temp_dir, f"output_{department}_{user_name}.pdf"
            )
    ...
    def _overlay_content_on_docx_template(
        ...
        try:
            ...
            output_path = os.path.join(
                self.pdf_generator.temp_dir, f"output_{department}_{user_name}.docx"
            )
    ...
    def _parse_content_to_blocks(self, content: str, metadata: dict) -> list:
        ...
        default_font = (
            list(metadata.get("fonts", {}).keys())[0]
            if metadata.get("fonts")
            else "helv"
        )
        default_size = (
            list(metadata.get("text_sizes", {}).keys())[0]
            if metadata.get("text_sizes")
            else 11
        )
    ...
    # -------------------------------------------------------------------------
    async def _prepare_pdf_files(
        self, user_valves, content, user_name, templates=None
    ) -> dict:
        try:
            from xhtml2pdf import pisa
        except ImportError:
        ...
        for department in departments_list:
            ...
            if (
                use_template
                and template_name
                and self.template_manager
                and self.pdf_generator
            ):
                    ...
                    for img_idx, img in enumerate(image_list):
                        ...
                        try:
                            ...
                            if image_rects:
                                ...
                                coverage = (img_rect.width * img_rect.height) / (
